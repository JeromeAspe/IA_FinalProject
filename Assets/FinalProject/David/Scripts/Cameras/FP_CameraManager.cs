...
public class FP_CameraManager : FP_Singleton<FP_CameraManager>, IHandler<string, FP_CameraBehaviour>
{
	...
	public void CreateCamera<TCamera>(string _id, FP_CameraSettings _settings, Transform _target = null) where TCamera : FP_CameraBehaviour
	{
		GameObject _camera = new GameObject($"Camera => {_id}", typeof(Camera));
		TCamera _cameraBehaviour = _camera.AddComponent<TCamera>();
		if (!_settings.Target && _target) _settings.SetTarget(_target);
		_cameraBehaviour.SetID(_id);
		_camera.transform.SetParent(_target);
		_cameraBehaviour.SetSettings(_settings);
		_camera.transform.position = _settings.TargetPosition;
	}
